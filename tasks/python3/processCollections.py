...
def validate_concept_id(concept_id):
  if concept_id is not "TBD" and concept_id is not "N/A" and concept_id is not None:
    return True

def format_output_dict():
  for val in product_dict.values():
    concept_id_val = val.get('concept-id')
    if validate_concept_id(concept_id_val):
      wv_id_val = val.get('wv-id')
